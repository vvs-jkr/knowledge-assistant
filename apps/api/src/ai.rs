...
\n- Используй раздел 'Примеры последних тренировок' как шаблон формата и стиля\
\n- Придерживайся функционального тренинга / CrossFit-стиля (bodyweight, гири, ruck)\
\n- Используй форматы: for_time, amrap, emom, tabata, rounds -- как в примерах\
\n- НЕ добавляй разминку и заминку если их нет в примерах\
