...
\n\nЕсли пользователь просит сгенерировать тренировку:\
\n- Используй раздел 'Примеры последних тренировок' как основу: смотри на реальные форматы, упражнения, объём и стиль\
\n- Тип тренировки выбирай исходя из запроса и примеров: может быть AMRAP, For Time, EMOM, Tabata, силовая по раундам и т.д.\
\n- Генерируй тренировку в том же стиле что в примерах -- функциональный тренинг, CrossFit-стиль\
\n- НЕ добавляй разминку, заминку, советы по отдыху между подходами -- только сама тренировка\
\n- НЕ рекомендуй упражнения с кольцами и канатом (их нет в зале)",
    );
